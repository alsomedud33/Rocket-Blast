		...
		"Melee":
			...
			for hb in NetNodes.players.get_node(str(id)).camera.get_node("Melee Hitbox").get_overlapping_areas():
				if hb.get_parent().is_in_group("Player") and hb.get_parent().name != id and hit == false:
					hb.get_parent().take_damage(damage,id,true)
...
func _explosion_hitbox(hitbox,players,damage):
	rpc_unreliable("_explosion_hitbox_remote",hitbox,players,NetNodes.hitboxes.get_node(hitbox).explosion_owner,round(damage * clamp(1/NetNodes.hitboxes.get_node(hitbox).get_global_transform().origin.distance_to(NetNodes.players.get_node(players).get_global_transform().origin),1,1.2)),NetNodes.hitboxes.get_node(hitbox).get_global_transform().origin,NetNodes.hitboxes.get_node(hitbox).explode_force,NetNodes.hitboxes.get_node(hitbox).y_explode_ratio,NetNodes.hitboxes.get_node(hitbox).distance_ratio,NetNodes.hitboxes.get_node(hitbox).translation)
	if (NetNodes.hitboxes.get_node(hitbox).explosion_owner == str(get_tree().get_network_unique_id())) and NetNodes.hitboxes.get_node(hitbox).explosion_owner != players:
		Network.emit_signal("hit",round(damage * 1/clamp(NetNodes.hitboxes.get_node(hitbox).get_global_transform().origin.distance_to(NetNodes.players.get_node(players).get_global_transform().origin),1,1.2)),NetNodes.players.get_node(players).head.global_transform.origin)
	print ("damage is:" + str(round(damage * 1/clamp(NetNodes.hitboxes.get_node(hitbox).get_global_transform().origin.distance_to(NetNodes.players.get_node(players).get_global_transform().origin),1,1.2))))
	NetNodes.players.get_node(players).take_damage(round(damage *1/clamp(NetNodes.hitboxes.get_node(hitbox).get_global_transform().origin.distance_to(NetNodes.players.get_node(players).get_global_transform().origin),1,1.2)),NetNodes.hitboxes.get_node(hitbox).explosion_owner)
